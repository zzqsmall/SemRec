function [ te_rmse, te_mae, weight ] = eff_SemRec_Col( dsname, dsratio, algname, fold, parameters, pathsetname, is_savemodel, model_filename )
    ...
    for i = 1:fold
        % training/test data partition, data saving/loading.
        if is_savemodel
            ord = randperm(rcnt);
    ...
    fprintf('%-30s : %.2e\n','learning rate',parameters(3));
    fprintf('%-30s : %.2e\n','eps',parameters(4));
    fprintf('%-30s : %d\n','max iteration times',parameters(5));
